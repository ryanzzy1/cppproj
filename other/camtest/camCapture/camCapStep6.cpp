...
int main()
{
    ...
    vfmt.fmt.pix.pixelformat = V4L2_PIX_FMT_MJPEG; // 设置为mjpeg格式，则我可以直接写入文件保存，YUYV格式保存后需要转换格式才能查看
    ...
    readbuffer.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;  // 每个结构体都需要设置type为这个参数，记住
    ret = ioctl(fd, VIDIOC_DQBUF, &readbuffer);
    if (ret < 0)
    {
        perror("放回队列失败");
    }
    FILE *file = fopen("my.jpg", "w+"); //打开文件
