        ...
        if ((info.instance_state == ALIVE_INSTANCE_STATE) && info.valid_data)
        {
            ...
            if (requests_status_.end() != request_status)
            {
                if (!request_status->second)
                {
                    ...
                    std::cout << "From request to server time: " << std::chrono::duration<double, std::micro>(time_resp - time_req).count() << " us\n";
                }
                else
                {
...
void ClientApp::create_participant()
{
    ...
    // omited thread configuration
    // set initial peers
    eprosima::fastdds::rtps::Locator_t initial_peer_locator;
    initial_peer_locator.kind = LOCATOR_KIND_TCPv4;
    eprosima::fastdds::rtps::IPLocator::setIPv4(initial_peer_locator, "172.16.30.49");
    eprosima::fastdds::rtps::IPLocator::setPhysicalPort(initial_peer_locator, 5100);
    // If logical port is set in the server side, it must be also set here with the same value
