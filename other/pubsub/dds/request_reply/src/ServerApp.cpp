    ...
    , stop_(false)
{
    ...
    request_reply_info("ServerApp", "Server initializated with ID: " << participant_->guid().guidPrefix);
}
        ...
        const std::string& service_name)
{
    ...
    {
        throw std::runtime_error("Request reader initilization failed");
    }
}
        ...
        const std::string& service_name)
{
    ...
    {
        throw std::runtime_error("Publisher initilization failed");
    }
...
void ServerApp::reply_routine()
{
    while (!is_stopped())
    {
        ...
        if (!is_stopped())
        {
            ...
            // If none of the client's endpoints ara matched, ignore the request as the client is gone
