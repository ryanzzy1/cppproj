...
public:
    ...
    void on_message(const std::shared_ptr< vsomeip::message > &_response) {
        time_resp = std::chrono::steady_clock::now();
        std::cout << "Received a response from Service ["
                ...
                << "]"
                << std::endl;
        std::cout << "From Request to Response time: " << std::chrono::duration<double, std::micro>(time_resp - time_req).count() << " us\n";
        // std::cout << "From Request to Response time: " << std::chrono::duration_cast<std::chrono::microseconds>(time_resp - time_req).count() << " us\n";
        if (is_available_)
            send();
        ...
        while (running_) {
            {
                ...
                if (is_available_) {
                    app_->send(request_);
                    time_req = std::chrono::steady_clock::now();
...
private:
    ...
    bool blocked_;
    bool is_available_;
    std::chrono::steady_clock::time_point time_req;
    std::chrono::steady_clock::time_point time_resp;
